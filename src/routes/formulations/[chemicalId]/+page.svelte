      ...
      <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
        <table class="min-w-full">
          ...
          <tbody class="bg-white">
            {#each data.data.chemical.Formulation as f (f.ID)}
              ...
              {#each f.Brand as brand (brand.ID)}
                {#each brand.Pack as pack (pack.ID)}
                  {@const priceDifference = (pack.Price ?? 0) - (pack.Subsidy ?? 0)}
                  <tr class="border-t border-gray-200">
                    <td
